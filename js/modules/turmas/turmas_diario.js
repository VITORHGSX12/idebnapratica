...
(function(global) {
    ...
    // -------------------------------------------------------------------------
    // -------------------------------------------------------------------------
    // 2. MODAL DO DIÁRIO DE CLASSE COM ABAS (ALUNOS, PROFESSORES, PLANEJAMENTO)
    // -------------------------------------------------------------------------

    var activeModalJournalTurmaId = null;
    var activeModalJournalTurmaNome = '';
    var activeModalJournalEscolaNome = '';
    ...
    function openTurmaJournalModal(turmaId, turmaNome, escolaNome) {
        ...
        activeModalJournalTurmaId = turmaId || 'turma_01';
        activeModalJournalTurmaNome = turmaNome || '5º Ano A';
        activeModalJournalEscolaNome = escolaNome || global.currentSelectedSchoolDetail || 'UI JOSE CORREA LIMA';

        if (typeof global.TurmasPlanejamento !== 'undefined' && global.TurmasPlanejamento.setCurrentTurmaNome) {
            global.TurmasPlanejamento.setCurrentTurmaNome(activeModalJournalTurmaNome);
        }

        var school = activeModalJournalEscolaNome;
        var className = activeModalJournalTurmaNome;

        if (title) title.textContent = 'Diário de Classe — ' + className;
        if (subtitle) subtitle.textContent = school + ' • Ano Letivo 2026 • Gestão Pedagógica & Desempenho';

        // Renderiza estrutura de abas internas no modal
        container.innerHTML = [
            '<div style="display: flex; gap: 8px; border-bottom: 2px solid var(--border-color); margin-bottom: 16px;">',
            '    <button type="button" id="tab-btn-journal-alunos" onclick="switchJournalInnerTab(\'alunos\');" class="btn btn-sm" style="background: var(--bg-tertiary); border: none; border-bottom: 2px solid var(--color-brand-primary); border-radius: var(--radius-sm) var(--radius-sm) 0 0; font-weight: 700; color: var(--color-brand-primary); padding: 8px 16px; display: flex; align-items: center; gap: 6px;">',
            '        <i data-lucide="users" style="width: 14px; height: 14px;"></i> Alunos & Desempenho',
            '    </button>',
            '    <button type="button" id="tab-btn-journal-professores" onclick="switchJournalInnerTab(\'professores\');" class="btn btn-sm" style="background: transparent; border: none; border-radius: var(--radius-sm) var(--radius-sm) 0 0; font-weight: 600; color: var(--text-secondary); padding: 8px 16px; display: flex; align-items: center; gap: 6px;">',
            '        <i data-lucide="graduation-cap" style="width: 14px; height: 14px;"></i> Professores da Turma',
            '    </button>',
            '    <button type="button" id="tab-btn-journal-planejamento" onclick="switchJournalInnerTab(\'planejamento\');" class="btn btn-sm" style="background: transparent; border: none; border-radius: var(--radius-sm) var(--radius-sm) 0 0; font-weight: 600; color: var(--text-secondary); padding: 8px 16px; display: flex; align-items: center; gap: 6px;">',
            '        <i data-lucide="calendar" style="width: 14px; height: 14px;"></i> Planejamento Pedagógico',
            '    </button>',
            '</div>',
            '<div id="modal-journal-tab-content-alunos">',
            '    <!-- Tabela de Alunos Injetada Abaixo -->',
            '    <div id="modal-journal-alunos-table-wrapper"></div>',
            '</div>',
            '<div id="modal-journal-teachers-container" class="hidden"></div>',
            '<div id="modal-journal-planejamento-container" class="hidden"></div>'
        ].join('\n');

        renderJournalAlunosList();
    ...
    function switchJournalInnerTab(tabName) {
        var btnAlunos = document.getElementById('tab-btn-journal-alunos');
        var btnProfs = document.getElementById('tab-btn-journal-professores');
        var btnPlan = document.getElementById('tab-btn-journal-planejamento');

        var paneAlunos = document.getElementById('modal-journal-tab-content-alunos');
        var paneProfs = document.getElementById('modal-journal-teachers-container');
        var panePlan = document.getElementById('modal-journal-planejamento-container');

        var buttons = [btnAlunos, btnProfs, btnPlan];
        var panes = [paneAlunos, paneProfs, panePlan];
        ...
        panes.forEach(function(p) { if (p) p.classList.add('hidden'); });

        if (tabName === 'alunos') {
            if (btnAlunos) {
                btnAlunos.style.background = 'var(--bg-tertiary)';
                btnAlunos.style.borderBottom = '2px solid var(--color-brand-primary)';
                btnAlunos.style.color = 'var(--color-brand-primary)';
                btnAlunos.style.fontWeight = '700';
            }
            if (paneAlunos) paneAlunos.classList.remove('hidden');
        } else if (tabName === 'professores') {
            if (btnProfs) {
                btnProfs.style.background = 'var(--bg-tertiary)';
                btnProfs.style.borderBottom = '2px solid var(--color-brand-primary)';
                btnProfs.style.color = 'var(--color-brand-primary)';
                btnProfs.style.fontWeight = '700';
            }
            if (paneProfs) paneProfs.classList.remove('hidden');
            if (typeof global.TurmasProfessores !== 'undefined' && global.TurmasProfessores.renderTurmaProfessoresTab) {
                global.TurmasProfessores.renderTurmaProfessoresTab(activeModalJournalTurmaId, 'modal-journal-teachers-container');
            }
        } else if (tabName === 'planejamento') {
            if (btnPlan) {
                btnPlan.style.background = 'var(--bg-tertiary)';
                btnPlan.style.borderBottom = '2px solid var(--color-brand-primary)';
                btnPlan.style.color = 'var(--color-brand-primary)';
                btnPlan.style.fontWeight = '700';
            }
            if (panePlan) panePlan.classList.remove('hidden');
            if (typeof global.TurmasPlanejamento !== 'undefined' && global.TurmasPlanejamento.renderTurmaPlanejamentoTab) {
                global.TurmasPlanejamento.renderTurmaPlanejamentoTab(activeModalJournalTurmaId, 'modal-journal-planejamento-container');
            }
        }
    ...
    function renderJournalAlunosList() {
        var wrapper = document.getElementById('modal-journal-alunos-table-wrapper');
        if (!wrapper) return;
        ...
        wrapper.innerHTML = [
            ...
            alunos.map(function(a) {
                ...
                return [
                    ...
                    '                        <button type="button" onclick="if(window.TurmasPlanejamento) window.TurmasPlanejamento.openCreatePlanModal(\'' + activeModalJournalTurmaId + '\', \'' + aId + '\');" class="btn btn-outline" style="font-size: var(--text-xs); padding: 4px 8px; color: var(--color-brand-primary);" title="Criar Plano Individual">+ Plano</button>',
                    '                    </div>',
                    '                </td>',
                    '            </tr>'
                ].join('\n');
    ...
    global.switchJournalInnerTab = switchJournalInnerTab;
